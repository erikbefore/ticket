	...
	<div class="card-body">
		...
		@if ($u->currentLevel() > 1)
			@if (session()->has('panichd_filters'))
				<a href="{{ action('\App\Http\Controllers\TicketsController@index') }}/filter/removeall" class="btn btn-light btn-sm tooltip-info removeall-link"  data-toggle="tooltip" title="{{ trans('panichd::lang.filter-removeall-title') }}" data-placement="bottom"><span class="fa fa-filter"></span></a>
			@endif

			@include('panichd::tickets.partials.filter_blocks')
		@endif

		{!! link_to_route($setting->grab('main_route').'.create', trans('panichd::lang.btn-create-new-ticket'), null, ['class' => 'btn btn-outline-success float-right']); !!}
